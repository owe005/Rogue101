...
public class Location {
	...
	public GridDirection directionTo(Location loc) {
		...
		GridDirection dDir = GridDirection.CENTER; //definerer dDir som center "retningen"
		
		int dist = gridDistanceTo(loc); //distance til location
		
		for(GridDirection dir : GridDirection.EIGHT_DIRECTIONS) { //for loop i 8 directions
			Location location = this.getNeighbor(dir); //location er nearby object i directions
			
			if(dist > location.gridDistanceTo(loc)) { 
				dist = location.gridDistanceTo(loc);
				dDir = dir;
			}
